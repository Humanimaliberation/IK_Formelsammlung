...
\usepackage{marvosym}			% Symbole, z.B. A im Kreis

\usepackage{longtable}			% Für Ascii Tabelle
...
Anmerkung: Layout und weiteres für eine freiere Wissensgesellschaft erklaut. \CircledA 
...
\begin{tabular*}{\columnwidth}{p{1.4cm} | p{2.2cm} | p{2.2cm}}
	...
	Sendeleistung (Signal) & $P_S$ & $[P_S] = 1 W $ oder $[P_{S,dBm}] =  1 dBm $ \\ \hline
	Rauschleistung (Noise) & $P_N$ & $[P_N] = 1 W $ oder $[P_{N,dBm}] = 1 dBm $ \\ \hline
	Interferenzleistung & $P_I$ & $[P_I] = 1 W $ oder $[P_{I,dBm}] = 1 dBm $ \\ \hline
...
\sectionbox{ Leitungstheorie \textbf{relevant für $l >= 0,1 \lambda$}
	...
	Wellenimpedanz $\underline{Z}_L = \sqrt{\frac{(R'+ j\omega L')}{(G' +j \omega C')}} $ \\
	Wellenlänge $\lambda$ mit [$\lambda$] = m \\
	Ausbreitungsgeschwindigkeit $v$ = $\lambda \cdot f$ mit [$v$] = m/s \\
	Ausbreitungskonstante $\gamma = \alpha + j \beta = \sqrt{(R' + j\omega L')(G' + j \omega C')}$ mit [$\gamma$] = $\frac{1}{m}$ \\
	...
	%TODO: Eventuell Grafik mit Methode aus HS für den zeitlichen Verlauf von reflektierenden Wanderwellen zwischen 2 Störstellen
...
\sectionbox{
	...
	$\underline{S} = S = \frac{1}{2}\underline{E} \cdot \underline{H}^{*}  \approx \frac{1}{2}\underline{H}^2 Z_{W0} = \frac{1}{2}\frac{Z_W0}{\underline{E}^2}$ mit $E, H \in \mathbb{C}$ \\ 

	
	(Feld-)Wellenwiderstand: $Z_W = \frac{E}{H} = \sqrt{\frac{\mu_0 \mu_r}{\epsilon_0 \epsilon_r}}$ \\
	$Z_{W0} = \sqrt{\frac{\mu_0}{\epsilon_0}} \approx 376,73 \Omega$  im Vakuum (näherungsweise in Luft) \\

	$S_i = \frac{P_S}{4 \pi r^2}$ Leistungsflussdichte isotroper Kugelstrahler\\
	...
	Bsp. Empfangs- und Sendeantenne im Abstand r zueinander: \\ $P_E = P_S \cdot G_S \cdot G_E (\frac{\lambda}{4 \pi r})^2 = S_r \cdot A_{W,E}$ \\
	
	\includegraphics[width = 6cm]{./img/Funkuebertragung.png}
}
...
Beispiel-Protokolle der verschiedenen Schichten in Anhang2. \\

\textbf{Beispiel-Trace und Header-Verkapselung:}\\

\includegraphics[width = 6cm]{./img/tcp_trace.png}

\subsection{Nutzung des Cheatsheet}
Allg.: Jede Spalte bei den Headerabbildungen entspricht 1 Bit. \\
Pro Zeile also 32 Bit bzw. 4 Byte = 4 zweistellige Hexzahlen \\

1. \textbf{Bei} Nutzung einer \textbf{Netzwerkkarte} wird \textbf{immer Ethernet} verwendet. Mit diesem Header für die Sicherungsschicht beginnt die Datenübertragung (s. beide Grafiken oben)\\ %TODO: Grafik-Referenz
...
\sectionbox{
\subsection{ALOHA, pure and slotted}
...
\sectionbox{
	\textbf{Bitstopfen}(Bit Stuffing) \\
	Dieses eingefügte Bit wird verwendet, wenn innerhalb der Nutzdaten zufällig die Bitfolge auftaucht, die als Blockbegrenzung (Flag) vorgesehen ist. \\
...
\sectionbox{
	\subsection{CRC} 
	\textbf{C}yclic \textbf{R}edundancy \textbf{C}heck\\
	Sender und Empfänger verwenden das gleiche Prüfpolynom! \\
	...
	\textbf{Unerkennbare Bitfehler} errechnen:\\
	Zusammenhängende Bitfolge in den Empfangsdaten in der Länge des Polynoms (oder das gesamte Polynom) durch XOR mit dem Polynom verknüpfen. Also Division weiterhin restfrei. \\
...
\sectionbox{
	\subsection{Informationsgehalt und Entropie (mittlerer Informationsgehalt)}
	$s_i$: Zeichen aus Alphabet mit $M$ Zeichen\\
	$I_i$: Informationsgehalt  des Zeichens $s_i$\\
	$p_i$: Wahrscheinlichkeit  des Zeichens $s_i$ \\
	$l_i$: Codewortlänge des Zeichens $s_i$ \\
	$H$: Entropie eines Codes als Erwartungswert des Informationsgehalt eines Zeichens aus dessen Alphabet.\\
	
	$I_i = - \log_2 p_i$  $\left[\ir{bit} \right]$ \\ 
	
	Entropie $ H = - \sum\limits_{i = 0}^{M-1} p_i \log_2 p_i $ 	$\left[\frac{\ir{bit}}{\ir {Zeichen}}\right]$\\
	$H_{max} =  - \log_2 \frac{1}{M}$,  wenn alle Zeichen gleich wahrscheinlich sind.	
	
	Mittlere Codewortlänge $\overline l  = \bar{N} = \sum \limits_{i= 0}^{n-1} p_i l_i$
	
	Die minimale mittlere Codewortlänge $\overline l \ge H$
}


\sectionbox{
	\subsection{Blockparität Bsp. 9 bit Nutzdaten, 7 bit Paritäten}
	\includegraphics[width = 2cm]{./img/Blockparitaet.png} 
	Nummerierung der Bits in Sendereihenfolge.\\



	P-Bits in der Spalte ganz rechts sind die Parität für alle in ihrer Zeile befindlichen Bits.\\
	P-Bits in der Zeile ganz unten sind die Parität für alle in ihrer Spalte befindlichen Bits.\\
	Beide Regeln werden auf das P-Bit am Ende angewendet. \\
	Gerade (Even) und Ungerade (Odd) Parität möglich.\\
}

\sectionbox{
	\subsection{Hamming Code(N,n)}
	$N$ Nachrichtenbits mit $ N = 2^k - 1 = n + k $\\
	$n$ Datenbits $n in \mathbb{N}$ \\
	$k$ Paritybits $k in \mathbb{N}$\\
	
	Beispielübertragung mit Hamming(7,4):\\
	\includegraphics[width = 6cm]{./img/HammingCode_bsp.png} \\
	(min.) Hamming-Distanz = 3, benachbarte Codewörter liegen also um 3 Bitänderungen außeinander.\\
	
	Allgemein: später mal... Beispiel (7,4) am anschaulichsten\\
	Sendereihenfolge (andere Bitbezeichnung wie oben):\\ P1, P2, D1, P4, D2, D3, D4(, ...)\\
	Paritätsbits liegen immer beim $(2^(k-1))$-ten Bit. \\
	%TODO: HAMMING ALLG?
}

\sectionbox{
	\subsection{Huffmann Code}
	Sortieren nach Wahrscheinlichkeit und Codewortlänge (kurzes Codewort \arrowOver hohe Wahrscheinlichkeit)\\
	Anleitung für manuelle visuelle Erstellung mittels eines "Huffman-Baums":\\
	1. Zeichen (und/oder Knoten) mit der niedrigsten Wahrscheinlichkeit durch Leitungen verbinden\\
	2. Beide Wahrscheinlichkeiten für die des Knotens addieren \\
	3. weiter mit 1. bis alle verbunden sind (Gesamtwahrscheinlichkeit 1) \\
	4. Für die neuen Codewörter nach Huffmann: rückwärts bei jeder Verzweigung eine 1 für den wahrscheinlicheren Zweig und eine 0 für den unwahrscheinlicheren \\
	5. Das erste Bit des Codeworts des Zeichens $x_i$ nach Huffmann ist die gesetzte 0 oder 1 bei der ersten Verzweigung hin zum Zeichen $x_i$. Das nächste Bit entsprechend der nächsten Verzweigung, usw.\\
	
	\includegraphics[width = 6cm]{./img/HuffmannCode_bsp.png}	
}


\sectionbox{
	\subsection{Reed-Solomon Code}
	k Daten werden in Zahlenformat übermittelt. Dazu kommen nochmals k Zusatz-Daten für die Fehlerkorrektur. Alle Daten liegen auf einem Polynom des Grades k-1. Fehler können korrigiert werden, wenn weniger als k Fehler auftreten. \\
	
	Bsp. Polynom ersten Grades also $k = 2$ \\
		Alle 4 Punkte müssen auf einer Geraden liegen, also mit gleichem Abstand zwischen einander. \\

		
	\includegraphics[width = 6cm]{./img/ReedSolomon_bsp.png} \\
	Der 2. oder 3. Zahlenwert in der Grafik ist fehlerhaft, da keine gleichen Abstände vorhanden sind.\\
	Richtiger Durchschnittsabstand zwischen allen Punkten (mit dem Fehler bei einem der mittleren Zahlenwerte):\\
	$\frac{163,5 - 17,4}{3} = \frac{41,1+56,3+48,7}{3} = 48,7 $
...
\sectionbox{
	...
	Informationsfluss:\\
	Info-Quelle \MVRightArrow Codierung \MVRightArrow Modulation \MVRightArrow Kanal \MVRightArrow Demodulation \MVRightArrow Decodierung \MVRightArrow Info-Senke \\
...
\sectionbox{
	\subsection{IPv4-Adressen (32 Bit Länge)}
	\textbf{IP-Adresse = Netzanteil (+ Subnetzanteil) + Hostanteil} \\
	\textbf{IP-Adresse UND (Sub-)Netzmaske = (Sub-)Netzwerkadresse} \\
	\textbf{(Sub-)Netzmaske} = alle Bits des (Sub-)Netzanteil gleich 1, alle Bits des Hostanteil gleich 0 \\
	...
	\textbf{CIDR-Wert}	Beispiel: 10.4.7.0/30 \\
	30 Bits für (Sub-)Netzanteil, und (32 - 30) Bit = 2 Bit  für den Hostanteil
	
	\includegraphics[width = 6.8cm]{./img/IP_Adressklassen.png} \\
	\includegraphics[width = 6.8cm]{./img/IP_Adressraum.png}\\
	...
	%TODO: Umschreiben IP-Adressen Cidr-Wert

\sectionbox{		
	\textbf{Beispiel Subnetting mit 3 Subnetzen und 62.0.0.0/8 zugewiesen}\\
	\includegraphics[width = 6.8cm]{./img/IPv4_bsp.png}\\
	
	Beispielrechnung: \\
	Für ausreichend Adressen für n Rechner werden aufgerundet $log_2(n+2)$ bit für den Hostanteil benötigt.\\
}

\sectionbox{
	\subsection{Router, Hubs, Switches}
%TODO Router, Hubs, Swithces
}
...
Außerdem: $\Phi \Xi \heartsuit$4\CircledA \quad Anmerkung s.u. \\ %Phi Xi Heart for Anarchy
...
Anmerkung zur Political Correctness (Stichwort Distanzieren):\\
Gegen: Sexismus, Gewalt an Mensch und Tier (zur Not mit Gewalt*), Herrschaft, denn Freiheit herrscht nicht, Rassismus, Nationalismus, Neoliberalismus/Kapitalismus, Diskriminierung aufgrund der Religion, des Aussehens, der physischen und psychischen Gesundheit und der sexuellen Interessen, Kriege, Abschottung, Umweltzerstörung \\
Für: Mehr Liebe und Anarchokommunismus (libertären Sozialismus), maximal selbstbestimmt (auch zivil ungehorsam) und solidarisch mit dezentralen basisdemokratischen Rätestrukturen, ein gutes Leben für alle\\

*Strategisches Kalkül: Wird dadurch mehr oder weniger Gewalt in Folge erzeugt? \\
"Redest du schon wieder pazifistisch verblendet, \\ 
reiben sich Faschisten die Hände!"[WTG - Gartenzaun] \\
...
\begin{tabular*}{\columnwidth}{l@{\extracolsep\fill}l}
	$ x = a \cdot log_{b}{(c \cdot d)} $ & $ b^{x} = (c\cdot d)^a = c^a \cdot d^a$ \\
	
	$ x = log_{b}{(c \cdot d)}^a$ & $ \sqrt[a]{(b^{x})} = c \cdot d $  \\	
	
	$ x = log_{b}{c}^a + log_{b}{d}^a $ & $log_{b}{b} = 1 $   \\
		
	$\log_{b1} x = \frac{\log_{b2} (x)}{\log_{b2} (b1)}$ &  $log_{b}{1} = 0$  \\
...
\sectionbox{
	...
	WS16/17, Umrechnung Phi, Wegdifferenz ... \\
	$e^{j\omega t + \phi} = \cos{\omega t + \phi} + j \sin{\omega t + \phi}$ \quad
	$|e^{j\omega t + \phi}| = 1$
	Funkwellenübertragung mit Wegdifferenz $\Delta s$ und Frequenz $f$ \\
	$\Delta \phi = \frac{\Delta s}{\lambda} = \frac{\Delta s \cdot f}{c}$ \\
}
...
\section{Stochastik}
% ============================================================================================
\sectionbox{KÜRZEN %TODO VEREINFACHEN/KÜRZEN
...
\\
	...
	Binominalkoeffizient $ \binom nk = \frac{n!}{k!\cdot(n-k)!} $ \\
...
% SECTION ====================================================================================
\section{Fourierreihe und -koeffizienten} %TODO: Beispiel-Fourierreihe
% ============================================================================================

\sectionbox{
	Fourierreihe f(t): Reihenentwicklung einer periodischen, abschnittsweise stetigen Funktion in eine Funktionsreihe aus Sinus- und Kosinusfunktionen\\
	$f(t) = \frac{a_0}{2} + \sum_{k=1}^{\infty}(a_k \cos(kt) + b_k \sin(kt)) = \sum_{-\infty}^\infty (c_ke_k(t))$ \\
	$a_k = \frac{1}{\pi} \int_{-\pi}^\pi f(t)\cdot \cos(kt) dt$ für $k \ge 0 $ \quad $k \in \mathbb{Z}$ \\
	$b_k = \frac{1}{\pi} \int_{-\pi}^\pi f(t)\cdot \sin(kt) dt$ für $k \ge 1 $\\
	$c_k = \frac{1}{2\pi} \int_{-\pi}^\pi f(t)e_{-k}(t) dt $ mit $c \in \mathbb{C}$ \\
	
	$f(t) = \frac{a_0}{2} + \sum_{k=1}^{\infty}(A_k \cos(kt-\phi_k))$\\
	$A_k = \sqrt{a_k^2 + b_k^2}$ \quad $A_k \cos(kt-\phi_k) = a_k \cos(kt) + b_k \sin(kt) $\\
	$a_k = A_k \cos(-\phi_k)$ \quad  $b_k = - A_k \sin(- \phi_k)$\\
	$\phi_k = arccos \frac{a_k}{A_k} = arcsin \frac{b_k}{A_k}$ \\
...
% SECTION ====================================================================================
\section{Fouriertransformation} %TODO: Kürzen?
% ============================================================================================
...
\sectionbox{
	\subsection{Lineare Ersatzspannungsquelle bestimmen}
	Bestimmung des Innenwiderstands $R_i$ und der Quellspannung $U_q$ einer linearen Ersatzspannungsquelle:\\
	
	1. $R_i$ = Widerstand von Klemme zu Klemme mit U-Quellen kurzgeschlossen und I-Quellen offen \\
	2. $U_q$ = Klemmspannung im Leerlauf \\
	
	\textbf{Beispiel-Ersatzspannungsquelle D}, identisch mit den Quellen A, B und C \\
	%TODO Grafik-Untertitel
	\includegraphics[width = 6.8cm]{./img/Ersatzspannungsquellen.png} \\
	
	Beispielrechnung:\\
	1.A: $R_i = 100 \Omega + (0 \Omega||100 \Omega) = 100 \Omega$ \\
	2.A: $U_q = 2V$ \\
	1.B: $R_i = 100 \Omega$ \\
	2.B: $U_q = \cdot \frac{100}{200} * 20 mA \cdot 200 \Omega = 2 V $ \\
	1.C: $R_i = 150 \Omega || 300 \Omega = 100 \Omega$ \\
	2.C: $U_q = 2,5 V + \frac{(1-2,5)V}{450 \Omega} \cdot 150 \Omega = 2 V$ \\
...
\sectionbox{
	\subsection{Transistorverstärker-Schaltung}
	\includegraphics[width = 6cm]{./img/Transistorschaltung.png} \\
	Die zum Beispiel dazugehörigen Aus- und Eingangskennlinien sind in den folgenden Subsections.\\
	
	Kennpunkte für Arbeitsgerade aus Beispielschaltung:\\
	Kurzschlussstrom für Innenwiderstand des Transistors gleich 0 mit  $I_K = \frac{U_b}{R_L} = 80 mA$ \\
	Leerlaufspannung $U_{CE}$ im Sperrbereich gleich $U_b = 16 V$ \\
	
}

\sectionbox{
	\subsection{Transistor Ausgangskennlinie und Arbeitsgerade}
	\includegraphics[width = 6cm]{./img/Transistor_Ausgangskennlinie.png} \\
	Die rote Arbeitsgerade hängt in der Beispielschaltung von $U_b$, $R_L$ ab.\\
	
	Markierter Arbeitspunkt auf Arbeitsgerade gewählt für minimales $I_{B,sat} = 150 \mu A$ damit $U_{CE}=U_{CE,sat}$, $U_{CE}$ also möglichst minimal bleibt.\\
	
	Bei einem höheren $I_B$ wäre der Arbeitspunkt im Sättigungsbereich und $U_{CE}$ höher. \\
}

\sectionbox{
	\subsection{Transistor Eingangskennlinie und Temperaturpunkt}
	\includegraphics[width = 6cm]{./img/Transistor_Eingangskennlinie.png} \\
	Temperaturbestimmung durch Schnittpunkt einer Temperaturgeraden mit $I_{Bsat}$ und $U_{BE} \approx 0,7 V$
}
...
\textbf{ASCII-Tabelle}
 ...
 \includegraphics[scale = 1.1]{./img/OSI_Modell3.png} \\
	...
	Unfertig, eigentlich als abgebbarer Teil für die Klausur gedacht, um Variablen einheitlicher zu bezeichnen. \\	
