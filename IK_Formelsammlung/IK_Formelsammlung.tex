...
\sectionbox{
	...
	3. weiter mit 2. bis alle verbunden sind (Gesamtwahrscheinlichkeit 1) \\
...
\begin{tabular*}{\columnwidth}{l@{\extracolsep\fill}ll}
	$b^a = x$ & $\log_b(1) = 0$ & $\log_b x = \frac{\log_a x}{\log_a b}$\\
	
	$log_b x = a$& $\log(10) = 1 $ & $\ln(\frac{x}{a}) = \ln x - \ln a$  \\
	
	$\ln(x^{c}) = c \ln(x)$ & $\ln e = 1$ & $\ln(x \cdot a) = \ln x + \ln a$\\
\end{tabular*}
}
