...
\end{tabular}
	...
	\subsection{Rechenregeln dB und dBm}
...
% SECTION ====================================================================================
\section{Baud-, Bit- und Übertragungsraten}
\sectionbox{
	\subsection{Definitionen}	
		Signalstufen		= Anzahl der möglichen annehmbaren Werte eines diskreten Signals pro Schritt \\
		binäre Datenmenge in bit (\textbf{b}inary dig\textbf{it}) mit bit-Anzahl $\in \mathbb{N}$ \\
		1 Byte [1B]		=	8 bit [8b] = $2^8$ \\
		Baudrate[Hz] 	= Schrittgeschwindigkeit \\
		Bitrate[bps] 	= $\frac{Bps}{8} = V \cdot$ Baudrate \\
...
\begin{tabular}{|c|c|c|}
	...
	Größe & Einheit & Def \\ 
	\hline 
	diskrete Signalstufen V & 1 & Anzahl möglicher annehmbarer Werte digitaler Signale \\ 
	\hline 
	Baudrate & 1 Hz & Schrittgeschwindigkeit \\ 
	\hline 
	Bitrate & $ 1 bps = 1 \frac{bit}{s}$ & Datenrate in bits $log_2{V} bit \cdot Baudrate$ \\ 
	\hline 
	binäre Datenmenge & 1 bit & 1 Byte = 8 bit \\ 
	\hline 
	&  &  \\ 
	\hline 
\end{tabular} 
...
\sectionbox{
	...
	Poynting-Vektor $\tilde{S}$ ist Vektor der Leistungsflussdichte mit $[S] = 1\frac{W}{m^2}$ \\
	...
	$S = \frac{1}{2}E \ast{H} = \frac{1}{2}H^2 Z_F = \frac{1}{2}\frac{Z_F}{E^2}$ mit $E, H \in \mathbb{C}$ \\ 
	...
	$S_i = \frac{P_S}{4 \pi r^2}$ \\
	
	Richtfaktor $D_i = \frac{S_{r,max}}{i} = 4 \pi r^2 \frac{S_{r,max}}{P_S}$\\
...
\sectionbox{
	Rahmenzeit $ \tau$ in $s$ für ein Paket \\
	Rate $ \lambda$ in $Hz$ mit der durschnittlich Pakete gesendet werden\\
	Zu sendende Pakete $ I $, bei keinen verlorenen Pakete gilt $ I = S $ \\
	...
	\subsection{CSMA/CD}
		...
		Ethernet:  Min. Nachrichtenlänge $ > $  Max. Konfliktdauer oder ev. künstl. Nachrichtenverlängerung mit Padding Bits \\
}
